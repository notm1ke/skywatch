...
const fetchTfrGeos = async () => okAsync(
	redis
...
type TfrTextUpstreamResponse = Array<{
	notam_id: string;
	text: string;
}>;

export type TfrTextResponse = {
	text: string;
};

export const fetchTfrText = async (notam_id: string) => okAsync(
	redis
		.get(`airspace:tfr:${notam_id}:text`)
		.then(raw => {
			if (raw) return safeParseJson<TfrTextResponse>(raw);
			return axios
				.get<TfrTextUpstreamResponse>(`https://tfr.faa.gov/tfrapi/getWebText?notamId=${encodeURIComponent(notam_id)}`)
				.then(res => res.data)
				.then(data => {
					if (!data.length) return { text: "Unavailable" };
					const text = data.at(0)!.text;
					
					redis.set(
						`airspace:tfr:${notam_id}:text`,
						JSON.stringify({ text }),
						"EX", TfrDataValidityPeriod
					);
