	...
	const onDataUpdate = React.useCallback(
		(updates: UpdateCell | Array<UpdateCell>) => {
			...
			for (let i = 0; i < tableRowCount; i++) {
				...
				if (updates) {
					const baseRow =
						existingRow ?? tableRow?.original ?? ({} as TData);
	...
	const onRowsDelete = React.useCallback(
		async (rowIndices: number[]) => {
			...
			requestAnimationFrame(() => {
				...
				const newRowCount =
					currentRows.length ?? propsRef.current.data.length;
				...
				case "left":
					if (isRtl) {
						if (currentColIndex < navigableColumnIds.length - 1) {
							const nextColumnId =
								navigableColumnIds[currentColIndex + 1];
				...
				if (!targetRow) {
					if (rowVirtualizer) {
						...
						if (newColumnId !== columnId) {
							requestAnimationFrame(() => {
								const cellKeyRetry = getCellKey(
									newRowIndex,
									newColumnId,
								);
								const targetCellRetry =
									cellMapRef.current.get(cellKeyRetry);
		...
		(event: KeyboardEvent) => {
			...
			if (
				...
				if (currentState.selectionState.selectedCells.size > 0) {
					for (const cellKey of currentState.selectionState
						.selectedCells) {
						const { rowIndex } = parseCellKey(cellKey);
						rowIndices.add(rowIndex);
					}
				...
				case "ArrowUp":
					if (altKey && !isCtrlPressed && !shiftKey) {
						...
						selectRange(selectionStart, {
							rowIndex: 0,
							columnId:
								navigableColumnIds[currentColIndex] ??
								selectionEdge.columnId,
						});
