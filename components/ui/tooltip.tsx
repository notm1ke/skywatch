...
function TooltipProvider({
	...
	const showTooltip = React.useCallback(
		(data: TooltipData) => {
			...
			const now = Date.now();
			const delay =
				now - lastCloseTimeRef.current < closeDelay ? 0 : openDelay;
...
function TooltipArrow({ className = "", style = {}, side }: TooltipArrowProps) {
	return (
		<div
			style={style}
			className={cn(
				...
				<TooltipPortal>
					...
					>
						<motion.div
							...
							transition={transition}
							className={cn(
								"bg-primary fill-primary text-primary-foreground relative w-fit rounded-md px-3 py-1.5 text-xs text-balance",
								currentTooltip.contentClassname,
							)}
							style={currentTooltip.contentStyle}
						>
							...
							{currentTooltip.arrow && (
								<TooltipArrow
									side={currentTooltip.side}
									className={currentTooltip.contentClassname}
									style={currentTooltip.contentStyle}
								/>
							)}
						</motion.div>
...
type TooltipContextType = {
	...
	setContentStyle: React.Dispatch<React.SetStateAction<React.CSSProperties>>;
	arrow: boolean;
...
function TooltipContent({
	...
	const { setContent, setContentClassname, setContentStyle, setArrow } =
		useTooltip();
